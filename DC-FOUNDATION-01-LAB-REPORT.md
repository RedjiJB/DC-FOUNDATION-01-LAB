...
**Status:** ✅ **COMPLETE** — Part A steps 0–12 executed, evidenced, and verified · SSH hardening proven by negative test (Fig 59) · reboot persistence confirmed · **all nine defects (D1–D9) closed** · published to `RedjiJB/cst-portfolio` · node reachable at static `172.20.10.10`
...
| D9 | Portfolio push | ✅ **Closed** — pushed to `RedjiJB/cst-portfolio`, README + evidence tracked, sensitive captures excluded and verified absent (§15.8) |
...
## 15.8 Portfolio publication — D9 closed

**Sequence:** `git branch -M main` (the repository was initialised on `master`; the push targeted `main`, which did not exist) → repository created on GitHub → push accepted.

```
Enumerating objects: 106, done.
Writing objects: 100% (103/103), 7.79 MiB | 409.00 KiB/s, done.
To github.com:RedjiJB/cst-portfolio.git
   fe1763c..d907cfc  main -> main
```

**`Repository not found` is not an authentication error.** The first push failed with that message despite `ssh -T` having greeted the account by name (Fig 41). GitHub deliberately returns *not found* rather than *permission denied* for repositories the caller cannot see, so it never confirms whether a private repo exists. Since authentication was already proven, the message meant exactly what it said: the remote repository had not been created. `git remote add` records a URL locally and creates nothing server-side.

### Evidence sanitisation — the control that mattered most

The screenshot folder held 98 images, of which three were **not** lab evidence:

| File | Content | Disposition |
|---|---|---|
| `Screenshot 2026-07-22 071704.png` | Online banking — account balances and ~46 transactions | Excluded |
| `Screenshot 2026-07-10 184342.png` | Unrelated coursework topology (CCNA/ROAS) | Excluded |
| `Screenshot 2026-07-10 183550.png` | Unrelated coursework topology (CCNA/ROAS) | Excluded |

**Why this had to be handled before the commit, not after.** Deleting a file in a later commit removes it from the working tree but **not from git history** — the blob remains in the pack and stays reachable by SHA on the remote even after the file "disappears" from the branch. Removing published sensitive data requires history rewriting (`git filter-repo`/BFG) plus a force push, and hosted caches may retain it regardless. Prevention is the only clean control.

The staging pattern used: copy the candidate set to a separate directory, remove the exclusions there, **review the remaining thumbnails by eye**, and transfer only the reviewed folder. The visual pass is the real control — the exclusion list only handles files already known to be a problem, while the review catches the ones nobody thought to list.

**✓ Verify — post-push confirmation that nothing sensitive was published:**

```bash
git ls-files | grep -E "071704|184342|183550"     # (no output — none tracked)
git ls-files | grep -i readme
L1-CST8207-linux/p00-node-foundation/README.md
```

Empty output on the first command is the proof: none of the three exist anywhere in the tracked tree. The report is present as `README.md`, and the repository is published.

> **Two operational notes.** (1) An earlier unfiltered `scp` had already placed all 94 images on the node; because **nothing had been committed at that point**, clearing `evidence/` and re-transferring the reviewed set was sufficient — the sanitisation window was still open. (2) Several published figures show the SSID, neighbouring BSSIDs, internal addressing, and SSH host-key fingerprints. None is sensitive in the way account data is, and all are normal in a lab writeup — but repository visibility should be a deliberate choice rather than a default.
...
- [x] Part A §11 Docker **without sudo** — D4 closed (Fig 51)
- [x] Part A §12 reboot-survival verification — D7 closed (Figs 53–54)
- [x] Defects D1–D9 all closed (§15.6, §15.7, §15.8)
- [x] Portfolio skeleton created, README + evidence committed and pushed
- [x] **Evidence reviewed for personal data before publishing** — 3 files excluded, absence verified post-push (§15.8)
- [x] Artifacts committed (Appendix A, B)
